...
superResolution <- function(LR_dir, HR_dir, modelList){
  ...
  for(i in 1:n_files){
    ...
    inds <- expand.grid(2:(dim(imgLR)[2]-1), 2:(dim(imgLR)[1]-1))[,2:1] %>% 
            matrix()
    ### step 1. for each pixel and each channel in imgLR:
    ###           save (the neighbor 8 pixels - central pixel) in featMat
    ###           tips: padding zeros for boundary points
    for (k in 1:3) {
      featMat[,,k] <- apply(inds, 1, findNeighb,imgLR[,,k] )
    ...
    prearray <- array(preMat, c(dim(imgLR)[1]*2,dim(imgLR)[2]*2,3))
    recover <- reallocate(prearray, x,y )
